...
if (isset($_POST['createJobPostBtn'])) {
    ...
    if (!empty($title) && !empty($description)) {
        ...
        $_SESSION['message'] = $result['message'];
        header("Location: ../hrDashboard.php"); // Adjusted to redirect to hrDashboard.php outside of core
        exit();
    } else {
        $_SESSION['message'] = "Please fill in all fields";
        header("Location: ../hrDashboard.php"); // Adjusted to redirect to hrDashboard.php outside of core
...
if (isset($_POST['applyJobBtn'])) {
    ...
    if ($resume['type'] !== 'application/pdf' || $resume['size'] > 5 * 1024 * 1024) { // 5MB limit
        ...
        header("Location: ../applicantsDashboard.php");
        exit();
    }
    ...
    $upload_dir = '../resumes/';  // Make sure the path is correct
...
if (isset($_POST['loginUserBtn'])) {
    ...
    if (!empty($username) && !empty($password)) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            if ($user['role'] === 'hr') {
                header("Location: ../hrDashboard.php"); // Redirect HR to HR dashboard
            } elseif ($user['role'] === 'applicant') {
                header("Location: ../applicantsDashboard.php"); // Redirect applicants to Applicant dashboard
            } else {
            ...
            $_SESSION['message'] = "User not found. Please check your credentials or create an account.";
            header("Location: ../login.php");
            exit();
        }
    } else {
...
if (isset($_POST['rejectApplicationBtn'])) {
    ...
    header("Location: ../viewApplications.php"); // Redirect to HR dashboard
    exit();
}
